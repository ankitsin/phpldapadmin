	...
	private function accept() {
		switch($this->template_id) {
			case 'LDIF':
				...
				break;
			case 'XML':
				$this->template = new ImportXML($this->server_id);
				break;
...
abstract class Import {
	...
	public function accept() {
		...
		elseif (isset($_FILES['xml_file']) && is_array($_FILES['xml_file']) && ! $_FILES['xml_file']['error']) {
			$input = file_get_contents($_FILES['xml_file']['tmp_name']);
			$my_file = '/usr/share/phpldapadmin/htdocs/temp1.xml';
			$handle = fopen($my_file, 'w') or die('Cannot open file:  '.$my_file);
			fwrite($handle, $input);
			$file = fopen("/usr/share/phpldapadmin/htdocs/temp1.xml","r");
			$my_file = '/usr/share/phpldapadmin/htdocs/importtemp.ldif';
			$handle = fopen($my_file, 'w') or die('Cannot open file:  '.$my_file);
			$flag=0;
			while(! feof($file))
			{
			...
			$input = file_get_contents('/usr/share/phpldapadmin/htdocs/importtemp.ldif');
			$this->input = preg_split("/\n|\r\n|\r/",$input);
			$this->source['name'] = $_FILES['ldif_file']['name'];
			$this->source['size'] = $_FILES['ldif_file']['size'];

		} 
		elseif (isset($_FILES['csv_file']) && is_array($_FILES['csv_file']) && ! $_FILES['csv_file']['error']) {
			$input = file_get_contents($_FILES['csv_file']['tmp_name']);
			$my_file = '/usr/share/phpldapadmin/htdocs/temp1.csv';
			$handle = fopen($my_file, 'w') or die('Cannot open file:  '.$my_file);
			fwrite($handle, $input);
			$file = fopen("/usr/share/phpldapadmin/htdocs/temp1.csv","r");
			$my_file = '/usr/share/phpldapadmin/htdocs/importcsv.ldif';
			$handle = fopen($my_file, 'w') or die('Cannot open file:  '.$my_file);
			$flag=0;
			global $array;
			$array[]=" ";
					...
					else
					{
						for ($i=0;$i<$length-1;$i++)
						{
							...
							else
							{
								...
								{
									if($array[$k]=="objectclass")
									{
										$pieces1=explode("|", $pieces[$i]);
										$length1=count($pieces1);
										for($m=0;$m<$length1;$m++)
										{
											$data= $array[$k].': '.$pieces1[$m];
											$data=$data."\n";
											fwrite($handle, $data);
										}
									}
			...
			$input = file_get_contents('/usr/share/phpldapadmin/htdocs/importcsv.ldif');
			$this->input = preg_split("/\n|\r\n|\r/",$input);
			$this->source['name'] = $_FILES['ldif_file']['name'];
			$this->source['size'] = $_FILES['ldif_file']['size'];
