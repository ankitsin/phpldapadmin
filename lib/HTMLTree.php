...
class HTMLTree extends Tree {
	...
	protected function draw_menu() {
		...
		$server = $this->getServer();
			...
			$link="createobject_form";
			$href_parms = htmlspecialchars(sprintf('cmd=%s&server_id=%s',$link,$server->getIndex()));
					if (isAjaxEnabled())
							$privatelinks.= sprintf('<br><a href="cmd.php?%s" onclick="return ajDISPLAY(\'%s\',\'%s\',\'%s\');" title="%s %s">%s</a><br>',
							$href_parms,'BODY',$href_parms,_('Loading Page'),$link,$server->getName(),"Create Class");
					else
						$privatelinks.= sprintf('<a href="cmd.php?%s" title="%s %s">%s</a>',
						$href_parms,$link,$server->getName(),$link);

			 //echo '<br><form><input type="button" value="Create Object Class" onClick="window.location.href=\'createobject_form.php\'"></form>';
			$link="createatt_form";
 			$href_parms = htmlspecialchars(sprintf('cmd=%s&server_id=%s',$link,$server->getIndex()));

					if (isAjaxEnabled())
							$privatelinks.= sprintf('<a href="cmd.php?%s" onclick="return ajDISPLAY(\'%s\',\'%s\',\'%s\');" title="%s %s">%s</a><br>',
							$href_parms,'BODY',$href_parms,_('Loading Page'),$link,$server->getName(),"Create Attribute");
					else
						$privatelinks.= sprintf('<a href="cmd.php?%s" title="%s %s">%s</a>',
						$href_parms,$link,$server->getName(),$link);

			 // echo '<br><form><input type="button" value="Add Attribute" onClick="window.location.href=\'createatt_form.php\'"></form>';
			$link="priority_form";
   			$href_parms = htmlspecialchars(sprintf('cmd=%s&server_id=%s',$link,$server->getIndex()));

					if (isAjaxEnabled())
							$privatelinks.= sprintf('<a href="cmd.php?%s" onclick="return ajDISPLAY(\'%s\',\'%s\',\'%s\');" title="%s %s">%s</a><br>',
							$href_parms,'BODY',$href_parms,_('Loading Page'),$link,$server->getName(),"ACL");
					else
						$privatelinks.= sprintf('<a href="cmd.php?%s" title="%s %s">%s</a>',
						$href_parms,$link,$server->getName(),$link);

			// echo '<br><form><input type="button" value="Access Control List" onClick="window.location.href=\'priority_form.php\'"></form><br>';
			// echo "shaghgdgjashdgaj";
 			echo $privatelinks;
			echo '</td></tr>';
		}
		/*if($privatelinks)
		{
			echo "agsgdhsafhjsajfj";
			echo $privatelinks;
		}*/
	}
	...
	protected function get_menu_item($item) {
		...
		switch($item) {
			...
			case 'createatt':
				if (!$_SESSION[APPCONFIG]->isCommandAvailable('script','createatt'))
					return '';

				$menu['cmd'] = 'createatt';
				$menu['ajax'] = _('Loading Attribute Form');
				$menu['div'] = 'BODY';
				$menu['title'] = _('Export');
				$menu['img'] = 'export-big.png';
				$menu['name'] = _('createatt');
