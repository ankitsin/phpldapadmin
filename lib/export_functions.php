...
class ExportXML extends Export{
	private $line_length = 76;

	static public function getType() {
		return array('type'=>'XML','description' => _('XML Export'),'extension'=>'xml');
	}
	...
	public function export(){
		...
		$file = "/usr/share/phpldapadmin/htdocs/exporttemp.ldif";
		$handle = fopen($file, 'w') or die('Cannot open file:  '.$file);
		/*$data= '<?xml version="1.0" encoding="UTF-8"?>';*/
		fwrite($handle, $output);
		$file = fopen("/usr/share/phpldapadmin/htdocs/exporttemp.ldif","r");
		$my_file = '/usr/share/phpldapadmin/htdocs/temp.xml';
		...
		$data="\n".'</root>';
		fwrite($handle, $data);
		$file = fopen("/usr/share/phpldapadmin/htdocs/temp.xml","r");
		while(! feof($file))
		{
			$output1 .= fgets($file);
		}
